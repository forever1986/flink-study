 ...
 * 演示reduce方法
 */
public class ReduceAndProcessDemo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 6. 计算
        SingleOutputStreamOperator<ServerInfo> reduce = windowStream.reduce(
                new ReduceFunction<>() {
                    @Override
                ...
                , new ProcessWindowFunction<>() {
                    @Override
                    public void process(String key, ProcessWindowFunction<ServerInfo, ServerInfo, String, TimeWindow>.Context context, Iterable<ServerInfo> elements, Collector<ServerInfo> out) throws Exception {
                        // 平均cpu值
                        long num = elements.spliterator().estimateSize();
                        ServerInfo serverInfo = elements.iterator().next();
                        System.out.println("服务器id=" + key + "在窗口的时间：" + DateFormatUtils.ISO_8601_EXTENDED_DATETIME_TIME_ZONE_FORMAT.format(context.window().getStart())
                                ...
                                + " 错误平均cpu值: " + (serverInfo.getCpu() / num));

                        out.collect(serverInfo);
                    }
                });
        // 7. 打印
        reduce.print("最终结果");
